 ...
 * <p>
 * Dashed stroke styles are not recommended with cloudy borders. The result would not look good because some parts of
 * the arcs are traced twice by the stroked path. Actually Acrobat Reader's line style dialog does not allow to choose a
...
class CloudyBorder
{
    ...
    /**
     * Creates a new <code>CloudyBorder</code> that writes to the specified content stream.
     *
     ...
     */
    CloudyBorder(PDAppearanceContentStream stream, double intensity, double lineWidth,
            PDRectangle rect)
    {
        this.output = stream;
    ...
    /**
     * Creates a cloudy border for a rectangular annotation. The rectangle is specified by the <code>RD</code> entry and
     * the <code>Rect</code> entry that was passed in to the constructor.
     * <p>
     * This can be used for Square and FreeText annotations. However, this does not produce the text and the callout
     * line for FreeTexts.
     *
    ...
    /**
     * Creates a cloudy border for a Circle annotation. The ellipse is specified by the <code>RD</code> entry and the
    ...
    /**
     * Returns the <code>BBox</code> entry (bounding box) for the appearance stream form XObject.
     *
    ...
    /**
     * Returns the updated <code>Rect</code> entry for the annotation. The rectangle completely contains the cloudy
     * border.
     *
    ...
    {
        return new PDRectangle((float) bboxMinX, (float) bboxMinY, (float) (bboxMaxX - bboxMinX),
                (float) (bboxMaxY - bboxMinY));
    }
    ...
    private void cloudyRectangleImpl(double left, double bottom, double right, double top,
            boolean isEllipse) throws IOException
    {
        double w = right - left;
        double h = top - bottom;
        ...
        if (w < 1.0)
        {
            polygon = new Point2D.Double[] { new Point2D.Double(left, bottom),
                    new Point2D.Double(left, top), new Point2D.Double(left, bottom) };
        }
        else if (h < 1.0)
        {
            polygon = new Point2D.Double[] { new Point2D.Double(left, bottom),
                    new Point2D.Double(right, bottom), new Point2D.Double(left, bottom) };
        }
        else
        {
            polygon = new Point2D.Double[] { new Point2D.Double(left, bottom),
                    new Point2D.Double(right, bottom), new Point2D.Double(right, top),
                    new Point2D.Double(left, top), new Point2D.Double(left, bottom) };
        }
    ...
    private void cloudyPolygonImpl(Point2D.Double[] vertices, boolean isEllipse) throws IOException
    {
        ...
        for (int j = 0; j + 1 < numPoints; j++)
        {
            ...
            int n = computeParamsPolygon(advIntermDefault, advCornerDefault, k, cloudRadius, length,
                    array);
            if (n < 0)
            {
                if (!outputStarted)
            ...
            addCornerCurl(anglePrev, angleCur, cloudRadius, pt.x, pt.y, alpha, alphaPrev,
                    !outputStarted);
     ...
     */
    private void addCornerCurl(double anglePrev, double angleCur, double radius, double cx,
            double cy, double alpha, double alphaPrev, boolean addMoveTo) throws IOException
    {
            ...
            double cy) throws IOException
    {
        ...
        getArcSegment(a + ANGLE_90_DEG, a + ANGLE_180_DEG - ANGLE_34_DEG, cx, cy, r, r, null,
                false);
    }
            ...
            throws IOException
    {
        ...
        getArcSegment(a + ANGLE_90_DEG, a + ANGLE_180_DEG - ANGLE_34_DEG, 0, 0, r, r, points,
                false);
    ...
    /**
     * Makes a polygon whose direction is the same as the positive angle direction in the coordinate system. The polygon
    ...
    /**
     * Returns the direction of the specified polygon. A positive value indicates that the polygon's direction is the
     * same as the direction of positive angles in the coordinate system. A negative value indicates the opposite
     * direction.
     *
     * The polygon must not intersect itself. A 2-point polygon is not acceptable. This is based on the "shoelace
     * formula".
     */
    ...
    /**
     * Creates one or more Bézier curves that represent an elliptical arc. Angles are in radians. The arc will always
     * proceed in the positive angle direction. If the argument `out` is null, this writes the results to the instance
     * variable `output`.
     */
    private void getArc(double startAng, double endAng, double rx, double ry, double cx, double cy,
            ArrayList<Point2D.Double> out, boolean addMoveTo) throws IOException
    {
        ...
        {
            getArcSegment(startAng + angleDone, startAng + angleDone + angleIncr, cx, cy, rx, ry,
                    out, false);
            angleDone += angleIncr;
            angleTodo -= angleIncr;
        }
    ...
    /**
     * Creates a single Bézier curve that represents a section of an elliptical arc. The sweep angle of the section must
     * not be larger than 90 degrees. If argument `out` is null, this writes the results to the instance variable
     * `output`.
     */
    private void getArcSegment(double startAng, double endAng, double cx, double cy, double rx,
            double ry, ArrayList<Point2D.Double> out, boolean addMoveTo) throws IOException
    {
     ...
     */
    private static Point2D.Double[] flattenEllipse(double left, double bottom, double right,
            double top)
    {
     ...
     */
    private double computeParamsEllipse(Point2D.Double pt, Point2D.Double ptNext, double r,
            double curlAdv)
    {
        double length = pt.distance(ptNext);
        if (Double.compare(length, 0.0) == 0)
